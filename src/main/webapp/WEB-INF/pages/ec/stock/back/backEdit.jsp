...
<div id="editPanel" class="myMainContent panel">
	...
	<form id="editForm">
		<div class="row">
			...
			<div class="col-sm-3">
				<div class="input-group">
					<span class="input-group-addon lable">归还名称</span>
			...
			<div class="col-sm-3">
				<div class="input-group">
					<span class="input-group-addon lable">归还时间</span>
					<input name="backDate" class="input-item form-control require" data-opt="{type:'date'}"/>
				</div>
			</div>
		</div>
		<div class="row mt10">
		...
		<div class="row mt10">
			<div class="col-sm-12 " style="border: 1px solid #ddd;">
				<table name="materialLeaseBackDetailInfos" class="input-entry" >
					<thead>
					<tr>
						...
						<th data-field="backCount" data-type="number" data-width="100">归还数量</th>
...
<script type="text/javascript">
    var editUI;
    var materialLeaseBackDetailInfosEntry;
    var materialLeaseBackDetailInfosEntryObj;
    function proSubItem_dataChange(oldData,newData){

    }
    function materialLeaseBackDetailInfos_dataChanged($cell,obj){
        if(webUtil.isEmpty(materialLeaseBackDetailInfosEntry)) return;
        if(obj.field=='materialInfo'&&!webUtil.isEmpty(materialLeaseBackDetailInfosEntry)){
            var materialInfoObj = obj.rowData[obj.field];
            materialLeaseBackDetailInfosEntry.setTableCellValue(obj.rowIndex, 'specification',
                materialInfoObj.specification);
            materialLeaseBackDetailInfosEntry.setTableCellValue(obj.rowIndex, 'materialType',
                materialInfoObj.materialType);
            materialLeaseBackDetailInfosEntry.setTableCellValue(obj.rowIndex, 'measureUnitInfo',
    ...
    $(document).ready(function() {
        var entryOption = "{type:'entry',height:250,tableOpt:{editDataChanged:materialLeaseBackDetailInfos_dataChanged}"+
            ",toolbar:{title:'材设归还清单'}}";
        $("table.input-entry").attr("data-opt",entryOption);
        editUI = $('#editPanel').editUI({
            title : "周转材料、设备使用(归还)",billModel:2,
            baseUrl : "ec/stock/back",
        ...
        materialLeaseBackDetailInfosEntryObj = editUI.getEntryObj('materialLeaseBackDetailInfos');
        if(!webUtil.isEmpty(materialLeaseBackDetailInfosEntryObj)){
            materialLeaseBackDetailInfosEntry = materialLeaseBackDetailInfosEntryObj.entry;
            var rightBtnGroup = materialLeaseBackDetailInfosEntryObj.toolbar.find('.pull-right>.btn-group').myBtnGroup();
            rightBtnGroup.addBtn({entry:materialLeaseBackDetailInfosEntry,css:'btn-sm',text:'复制插入',icon:"fa fa-edit",clickFun:btnCopyInsertRow});
            materialLeaseBackDetailInfosEntry.resetView();
        }
