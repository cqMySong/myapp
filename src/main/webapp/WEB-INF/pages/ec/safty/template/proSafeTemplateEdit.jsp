		...
		<div class="row">
			...
			<div class="col-sm-3">
				<div class="input-group">
					...
					<input type="text" name="safeTemplateInfo" class="form-control input-item"
						   data-opt="{type:'f7',dataChange:changeSafeTemplate,uiWin:{title:'样板标准',height:600,width:800,url:'ec/basedata/safetemplatef7',uiParams:getParamsTemplate}}">
				</div>
			</div>
			<div class="col-sm-3">
				<div class="input-group">
...
<script type="text/javascript">
    ...
    function changeSafeTemplate(oldData,newData) {
        if(newData){
            $('#showJobRequire').children().remove();
            webUtil.ajaxData({url:'ec/basedata/safetemplates/jobrequire',data:{safeTemplateId:newData.id},
    ...
    function afterAction(_opt,data){
        if(_opt==OperateType.addnew){
            ...
            webUtil.ajaxData({url:'ec/safe/template/query/job',data:{proSafeTemplateId:data.data.id},
                async:false,success:function(data){
                    if(data.statusCode=="0"){
                        $(data.data).each(function(i,v){
                            var str = '<div class="col-sm-6">' +
    ...
    $(document).ready(function() {
        editUI = $('#editPanel').editUI({
            title : "项目安全样板职责",billModel:2,
            baseUrl : "ec/safe/template",
