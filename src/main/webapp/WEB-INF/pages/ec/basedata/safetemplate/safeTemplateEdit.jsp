	...
	<form id="editForm">
		...
		<div class="row mt10">
			<div class="col-sm-12 " style="border: 1px solid #ddd;">
				<table name="safeTemplateDetailInfos" class="input-entry" >
					<thead>
					<tr>
...
<script type="text/javascript">
    var editUI;
    var safeTemplateDetailInfosEntry;
    var safeTemplateDetailInfosEntryObj;
    function safeTemplateDetailInfos_dataChanged($cell,obj){
        if(webUtil.isEmpty(safeTemplateDetailInfosEntry)) return;
        if(obj.field=='position'){
            if(!webUtil.isEmpty(safeTemplateDetailInfosEntry)){
                ...
                if(positionDetailArr&&positionDetailArr.length>0){
                    ...
                    $.each(positionDetailArr,function(i,positionInfo){
                        if(i==0){
                            safeTemplateDetailInfosEntry.setTableCellValue(obj.rowIndex,'enable',true);
                            positionInfoFirst = positionInfo;
                        }else{
                            var rowData = {position:{id:positionInfo.id,name:positionInfo.name},
                                enable:true};
                            safeTemplateDetailInfosEntry.insertRow(obj.rowIndex+i,rowData);
                        }
                    });
                    safeTemplateDetailInfosEntry.setTableCellValue(obj.rowIndex,'position',
    ...
    $(document).ready(function() {
        ...
        var entryOption = "{type:'entry',height:"+height+",tableOpt:{editDataChanged:safeTemplateDetailInfos_dataChanged}"+
            ",toolbar:{title:'工作要点明细'}}";
        $("table.input-entry").attr("data-opt",entryOption);
        editUI = $('#editPanel').editUI({
            title : "安全样板工作要点",billModel:1,
            baseUrl : "ec/basedata/safetemplate",
        ...
        safeTemplateDetailInfosEntryObj = editUI.getEntryObj('safeTemplateDetailInfos');
        if(!webUtil.isEmpty(safeTemplateDetailInfosEntryObj)){
            safeTemplateDetailInfosEntry = safeTemplateDetailInfosEntryObj.entry;
            var rightBtnGroup = safeTemplateDetailInfosEntryObj.toolbar.find('.pull-right>.btn-group').myBtnGroup();
            rightBtnGroup.addBtn({entry:safeTemplateDetailInfosEntry,css:'btn-sm',text:'复制插入',icon:"fa fa-edit",clickFun:btnCopyInsertRow});
            safeTemplateDetailInfosEntry.resetView();
        }
