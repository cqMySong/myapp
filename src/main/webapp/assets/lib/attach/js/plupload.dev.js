...
plupload.Uploader = function(options) {
	...
	function onUploadFile(up, file) {
		...
		/***
		 * @author songJun 需要重写此处的便宜 以便为了下次上传的续传
		 * offset = 102; 
		 */
		...
		function uploadNextChunk() {
			...
			if (up.settings.multipart && features.multipart) {
				...
				formData.append("formatSize", file.formatSize);
				formData.append("lastModifiedDate", file.lastModifiedDate.getTime());
				formData.append("md5", file.md5);//文件 依据文件名 修改时间 大小 共同确定的md5值 为了判断是否续传等相关操作
				
				if(up.settings.getOtherPrams){
					up.settings.getOtherPrams(formData);
				}
				//end
...
plupload.File = (function() {
	...
	function PluploadFile(file) {
		 ...
		 * 扩展文件初始化的其他一些信息
		 */
		plupload.extend(this,plupload._initNewFile(file));
		//end
